...
- [[Agentic Data Stack — Competitive Landscape + Market Clustering]] **★ THE MARKET MAP** — synthesis of 4 parallel research reports (May 2026) covering Snowflake Cortex / Databricks AI-BI / dbt+Fivetran+BigQuery / Postgres+Independents+AI-native startups. **5 strategic clusters**: Megaplatform Bundlers (Snowflake/Databricks/BigQuery) / Transformation+Orchestration (dbt-Labs+Fivetran/Astronomer/Dagster/Airbyte/dlt) / Modern BI+Notebooks (Hex/ThoughtSpot/Cube/Omni/Sigma/Lightdash/MotherDuck) / **AI-Native Data Verification (our cluster — Datafold/Recce/dbt-Labs-internal)** / Postgres-for-Agents (Lakebase/Snowflake-PG/Supabase/Neon). **Top threats ranked**: 1) Datafold (CRITICAL — same "binary verifiable outcomes" framing) 2) dbt Labs (HIGH — could ship "dbt Trust" at Coalesce Sept 15-18) 3) Recce (HIGH — OSS velocity + Jan 2026 Claude plugin) 4) Fivetran-via-SQLMesh. **8 consolidation predictions** (dbt Labs likely acquires Datafold AND/OR Recce in 12-18 months). **The wedge is REAL but narrowing — outcome-priced + policy-as-code + Receipt + SLA + vendor-neutral + benchmark-credentialed = no competitor ships all 6.** Action: plant standards flag (OSS Receipt spec + policy bundles) within 30 days; lock differentiator language ("not a diff tool, not LLM-judge eval, not notebook assistant"); ship MLP with Spider 2.0 #1 demo before Sept 15.
- [[P(PMF) — Honest Probability + Maximization Levers]] **★ THE LOAD-BEARING PAGE** — honest assessment of where we stand and what to do. Today P(PMF in 13 weeks) ≈ 25-35% on current trajectory; +30 percentage points purchasable by execution discipline + aggressive iteration. **10 levers ranked by probability gain per unit effort** (talk to 30+ users, sell before building, cut MLP scope, Sean Ellis test from day 1, pre-stage 4 named pivots, pay for distribution access $10K/qtr, ban Phase 2/3 in standups, weekly pivot/persevere gates, real scarcity in offers, Hormozi value-equation analysis). **7 things to STOP doing** (no new wiki pages, no parallel dashboard MCP build, no ed25519 spec'ing pre-customer-ask, etc.). **30-day calendar week-by-week** with concrete pivot gates. **Single sentence: "Talk to more users, build less, sell before shipping, kill scope ruthlessly, schedule pivot gates, and ban Phase 2 conversation. Everything else is wallpaper."**
- [[Stress Test — Lean / Zero-to-One / Hard Things]] **★ THE BRUTAL AUDIT** — wiki run against 3 operator/founder lenses. Lean Startup catches castles-in-air (validated learning loop too long; "5 design partners signed" is vanity; no pivot gates; receipts schema overbuilt). Zero to One catches commodity positioning (4/7 honest yes on the 7-question test; monopoly mechanism partially hand-waved; Secret fragile; distribution-as-moat unclear). Hard Things catches operational immaturity (no founder-market-fit articulation; no positioning canvas; no 11-star exercise; no scarcity; wartime tone missing; "do things that don't scale" not principled). **The 5 most uncomfortable questions** the wiki doesn't answer well. **Top 10 corrective actions** ranked by impact × effort. The 5-day "Pure-Concierge-Week" before MLP is the highest-leverage move.
- [[Receipt Product Features — Policy-as-Code, Action Loop, Honest Score, Vendor-Neutral Expansion]] **★ THE FEATURE SPEC** — codebase-grounded product features for The Receipt. Names the actual gap (verifier produces text not artifact; governance audit log + verifier outcomes live in different worlds). **Policy-as-Code thesis**: policy isn't a check, it's a contract. YAML schema replacing prompt-encoded rules. 4 named bundles (production-strict / dev-permissive / ci-gate / audit-trail-only). Concrete JSON Receipt artifact. PR comment UX (ASCII mockups for pass + block cases). **3-tier action loop** (Tier 1 auto-fix, Tier 2 delegate to Claude Code with context, Tier 3 structured override with auto-rollback). **Non-gimmicky score** (deterministic factor formula; calibration target = 90% no-incident-in-30d; receipt always shows factors[]). **Vendor-neutral adapter architecture** (~6-method contract; expansion order DuckDB → Snowflake → Databricks → BigQuery without protocol changes). 4-week MVP scope cut (~24 eng days, fits 5-engineer team) + post-MLP defer table. **+ Reference implementations in `spec/`** — Python verifier protocol, JSON Schema, 4 policy YAML bundles, PR comment Markdown templates.
...
- [2026-05-07 — Snowflake Cortex agentic stack research](raw/2026-05-07_research_snowflake-cortex-stack.md)
- [2026-05-07 — Databricks AI/BI/Genie/Mosaic agentic stack research](raw/2026-05-07_research_databricks-agentic-stack.md)
- [2026-05-07 — dbt Labs + Fivetran (post-merger) + BigQuery + Looker research](raw/2026-05-07_research_dbt-fivetran-bigquery-stack.md)
- [2026-05-07 — Postgres + Independent ELT/BI + AI-native data startups research](raw/2026-05-07_research_postgres-independents-agentic-stack.md)

---

## Reference implementation specs (`spec/`)

Buildable artifacts for the engineering team. Reference specs that bridge the wiki strategy to actual code. Not production code; refined when implementation begins.

- [`spec/README.md`](spec/README.md) — purpose, status, migration path
- [`spec/dbt_verifier_protocol.py`](spec/dbt_verifier_protocol.py) — Python interface stubs; `WarehouseAdapter` contract; `run_verifier()` orchestrator; `compute_score_v0()` deterministic score formula
- [`spec/receipt_v1_schema.json`](spec/receipt_v1_schema.json) — JSON Schema (draft-07) for Receipt v1
- [`spec/policy_bundles/production-strict.yaml`](spec/policy_bundles/production-strict.yaml) — production default; every check required, blocks on failure, override needs role + rollback
- [`spec/policy_bundles/dev-permissive.yaml`](spec/policy_bundles/dev-permissive.yaml) — dev sandbox; checks as warnings, save proceeds
- [`spec/policy_bundles/ci-gate.yaml`](spec/policy_bundles/ci-gate.yaml) — CI/CD; required checks block merge, JSON receipt as PR comment, no auto-commit
- [`spec/policy_bundles/audit-trail-only.yaml`](spec/policy_bundles/audit-trail-only.yaml) — observability mode; all checks run, none block, full audit log
- [`spec/pr_comment_templates.md`](spec/pr_comment_templates.md) — Markdown templates (PASS / WARN / BLOCK / short-update); A/B testing notes; CLI counterpart
